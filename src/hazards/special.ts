...
/**
 * Gravity loop: a big vertical ring the ball rides around the inside of.
 * Implemented with local track frames — a ring of box segments plus a magnet
 * field that redirects gravity radially outward (onto the inner surface).
 * The world never rotates.
 */
export class LoopField extends Hazard {
  private field: MagnetField;
  private colliders: BoxCollider[] = [];
  private center: THREE.Vector3;
  private normal: THREE.Vector3; // loop plane normal (horizontal)
  private inPlane1: THREE.Vector3; // horizontal in-plane dir
  private inPlane2 = new THREE.Vector3(0, 1, 0);
  ...
  constructor(
    ...
    super(id, ctx);
    this.center = new THREE.Vector3(...c);
    // plane contains travel direction (yaw) and up; normal is sideways
    const travel = new THREE.Vector3(-Math.sin(yaw), 0, -Math.cos(yaw));
    this.normal = travel.clone().cross(new THREE.Vector3(0, 1, 0)).normalize();
    this.inPlane1 = travel;

    // visual: big torus landmark
    const torusGeo = new THREE.TorusGeometry(r + 0.6, w / 2, 12, 42);
    const torus = new THREE.Mesh(torusGeo, ctx.mats.accent);
    torus.position.copy(this.center);
    torus.rotation.y = yaw + Math.PI / 2;
    torus.castShadow = true;
    ctx.group.add(torus);

    // inner track segments (checker ribbon)
    const segs = 22;
    for (let i = 0; i < segs; i++) {
      const a = (i / segs) * TAU;
      const segLen = ((TAU * r) / segs) * 1.15;
      const pos = this.pointAt(a, r + 0.25);
      const col = new BoxCollider(`${id}-seg${i}`).setBox(pos.x, pos.y, pos.z, segLen, 0.5, w);
      // orient: box "up" (local +Y) points toward loop center
      const inward = this.center.clone().sub(pos).normalize();
      const tangent = this.tangentAt(a);
      const m = new THREE.Matrix4().makeBasis(tangent, inward, this.normal);
      col.setQuat(new THREE.Quaternion().setFromRotationMatrix(m));
      ctx.world.add(col);
      this.colliders.push(col);
      // visual ribbon segment
      const segGeo = new THREE.BoxGeometry(segLen, 0.5, w);
      const segMesh = new THREE.Mesh(segGeo, ctx.mats.checkerTop);
      segMesh.position.copy(pos);
    ...
    this.field = {
      ...
      sample: (pos: THREE.Vector3, out: THREE.Vector3): number => {
        _v.copy(pos).sub(center);
        const side = _v.dot(normal);
        if (Math.abs(side) > ww / 2 + 1.2) return 0;
        _v.addScaledVector(normal, -side); // project into loop plane
        const radial = _v.length();
        if (radial < rr * 0.3 || radial > rr * 1.05) return 0;
        const upness = _v.y / Math.max(radial, 1e-4);
        // the bottom mouth is always open (normal gravity handles entry/exit)
        if (upness < -0.55) return 0;
        // too slow to loop: release in the lower half so the ball settles out
        if (upness < 0.15 && player.vel.lengthSq() < 49) return 0;
        out.copy(_v).normalize(); // gravity points radially OUTWARD onto the inner surface
        return clamp01((radial - rr * 0.3) / (rr * 0.35)) * 0.9;
      },
    };
    ctx.world.magnets.push(this.field);
  }

  private pointAt(angle: number, radius: number): THREE.Vector3 {
    return this.center
      .clone()
      .addScaledVector(this.inPlane1, Math.cos(angle) * radius)
      .addScaledVector(this.inPlane2, Math.sin(angle) * radius);
  }

  private tangentAt(angle: number): THREE.Vector3 {
    return this.inPlane1
      .clone()
      .multiplyScalar(-Math.sin(angle))
      .addScaledVector(this.inPlane2, Math.cos(angle))
      .normalize();
  }

  override update(): void {
    /* static geometry; the field does the work */
  }
